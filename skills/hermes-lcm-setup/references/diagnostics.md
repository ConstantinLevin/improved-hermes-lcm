...
- An `instruction_not_delivered` event in the doctor's recent events, with an ERROR in the log: the plugin could not give the host its system-prompt section at registration (the host offers none to it, its limit cannot be read, or the text is empty or over its limit), so the agent is not told what its summaries are.
- A WARNING in the host's log, "LCM's instruction is not in the system prompt of host session …": a request of a home whose context engine is LCM went out without the section. It names the host session, that fact, and the plugin sections the host's own parser finds in that prompt with their sizes; it names no cause. The host's log says whether it skipped the section (its total of 8,000 characters for all plugin sections), and a session the host restored from its database keeps the sections of its stored prompt until the next compaction. The same kind of WARNING says when a request carries no system-level prompt, and, once per agent, when the runtime (api_mode `codex_app_server`) bypasses the check.
