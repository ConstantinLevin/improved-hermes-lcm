...
- An `instruction_not_delivered` event in the doctor's recent events: the agent is not told what its summaries are. Either the plugin could not give the host its system-prompt section (the host offers none to it, its limits cannot be read, or the text is over its limit), or a request of an LCM session went out without the section. The event's detail names the host session, that fact, and the plugin sections the host's own parser finds in that prompt with their sizes; it names no cause. The host's log says whether it skipped the section (its total of 8,000 characters for all plugin sections), and a session the host restored from its database keeps the sections of its stored prompt until the next compaction.
